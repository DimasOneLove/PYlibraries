...
## MultiIndex - специальный класс Пандас
...
pop_df = pop.unstack() # Series -> DF
print(pop_df)
print(pop_df.stack()) # DF -> Series
...
print(data)
# print(data['a':'b'])# error - список не сортирован
...
print (pop.unstack())
# Перегруппировка Мультииндексов в желаемые последовательности
...
#print (pd.concat([ser1, ser2])) 
print (pd.concat([ser1, ser2], verify_integrity=False)) # Указываем, выдавать ли ошибку в случае повторяющихся индексов
print (pd.concat([ser1, ser2], ignore_index=True)) # отвечает сам за индексы
print (pd.concat([ser1, ser2], keys=['x', 'y'])) # самостоятельно создаем Мультииндекс
