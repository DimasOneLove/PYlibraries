...
# args = dict(alpha=0.3, bins=40) # параметры
...
# plt.hist2d(x, y, bins=30) # квадратные бины
# plt.hexbin(x, y, gridsize=30) # шестиугольные бины
# cb = plt.colorbar()
# cb.set_label("Point in the interval")
...
# ax.plot(x, np.cos(x) + 2) # по умолчанию в легенду не попадет
# ax.axis("equal")
...
# y = np.sin(x[:, np.newaxis] + np.pi * np.arange(0, 2, 0.5)) # двумерная функция
# lines = plt.plot(x, y)  # plt.Line2d
# plt.legend(lines, ["1", "второй", "third", "4-ый"], loc='upper left')

# plt.legend(lines[:2], ["1", "2"]) # дали названия не всем линиям



## Работа с датасетами
# cities = pd.read_csv("./Lesson 5-8 (Matplotlib)/data/california_cities.csv")
...
# plt.scatter([],[], c='k', alpha=0.5, s=100, label='100 $km^2$') # накладываем на график пустой график с label'ами для легенды
...
#     ax, lines[1:], ["line 2", "line 3", "line 4"], loc="lower left"
# )
...
# 3- качественные (смешиваются без четкого порядка)
...
# plt.subplot(1,2,2)
# plt.imshow(y, cmap=plt.cm.get_cmap('viridis', 6)) # сделали дискретным
# plt.colorbar()
# plt.clim(-0.25, 0.25) # предел
...
# ax2 = plt.axes([0.4, 0.3, 0.2, 0.1]) # Другая СК внутри исх СК
...
for i in range(1, 7):
    ax = fig.add_subplot(2, 3, i) # 2 строки 3 столбца i номер
